...
inline int tran(const char x) {
	if (x <= '9' && x >= '0') return x - '0';
	if (x <= 'Z' && x >= 'A') return x - 'A' + 10;
	return x - 'a' + 36;
}

inline char conv(const int x) {
	if (x >= 0 && x <= 9) return x + '0';
	if (x >= 10 && x <= 35) return x - 10 + 'A';
	return x - 36 + 'a';
}

inline void decimalize(const int b) {
	int i, j, k;
	x = 0;
	for (i=0; s[i]; ++i) {
		x = x * b + tran(s[i]);
...
inline void obaselize(const int b) {
	int i, j, k;
	p = 0;
	do {
		k = x % b;
...
inline void output(const int ib, const int ob) {
	printf("%d %s\n", ib, s);
	printf("%d ", ob);
	for (--p; p>=0; --p) printf("%c", t[p]);
	puts("\n");
}
