...
void getline(const point &x, const point &y, double &a, double &b, double &c){//两点x、y确定一条直线a、b、c为其系数
    a = y.y - x.y;
    b = x.x - y.x;
    c = y.x * x.y - x.x * y.y;
}
...
point intersect(const point &x, const point &y, const double a, const double b, const double c){//求x、y形成的直线与已知直线a、b、c、的交点
...
void cut(double a, double b , double c){
    ...
    for(int i = 1; i <= cCnt; ++i){
        if(a*p[i].x + b*p[i].y + c >= 0) {
			q[++curCnt] = p[i];// c由于精度问题，可能会偏小，所以有些点本应在右侧而没在，
        //故应该接着判断
        } else {
...
int main() {
    ...
    while (scanf("%d", &m), m) {
        ...
        solve();
        printf("Floor #%d\n", ++cn);
        if(cCnt>=1) puts("Surveillance is possible.");
