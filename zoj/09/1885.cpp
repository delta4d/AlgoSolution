...
inline bool gao(LL t, LL a, LL b) {
	LL i, j, k;
	LL n, m;
	bignum temp;
	bignum res;
	...
	if (t == 1) {
			...
			if (a % b == 0) {
				...
				if (n > 100) {
					...
					if (res.length() > 100) return false;
...
int main() {
	...
	while (scanf("%lld %lld %lld", &tt, &aa, &bb) != EOF) {
		if (!gao(tt, aa, bb)) printf("(%lld^%lld-1)/(%lld^%lld-1) is not an integer with less than 100 digits.\n", tt, aa, tt, bb);
	}
	return 0;
}