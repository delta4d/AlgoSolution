...
inline void gen_ten() {
	int i, j, k;
	ten[0] = 1;
	for (i=1; i<1000; ++i) ten[i] = ten[i-1] * 10;
}

inline bool suc(const int n, const int xn, const int &A, bignum &x) {
	//cout << ten[n] << ' ' << xn << ' ' << A << endl;
	if (ten[n] < A) return false;
	x = (ten[n] - A) * xn / (10 * A - 1);
	return x * (10 * A - 1) == (ten[n] - A) * xn;
}
...
void gen_f() {
	...
	for (A=2; A<MAXN; ++A) {
		for (n=1; 1; ++n) {
			for (xn=1; xn<10; ++xn) {
				//A = 4, n = 5, xn = 1;
				if (suc(n, xn, A, x)) goto done;
			}
		}
		done:;
		//if (A == 4) printf("n:%d xn:%d A:%d\n", n, xn, A);
		f[A] = ten[n] * xn + x;
	}
}
