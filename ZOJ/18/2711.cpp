...
inline bool legal(int l, int a, int b) {
	int c(l-a-b);
...
void gao() {
	int a, b, c, l;
	for (l=0; l<MAXN; ++l) for (a=0; a<MAXN; ++a) for (b=0; b<MAXN; ++b) f[l][a][b] = 0;
	f[0][0][0] = 1;
	for (l=1; l<=180; ++l) {
		for (a=0; a<MAXN; ++a) {
			for (b=0; b<MAXN; ++b) {
				c = l - a - b;
				if (c >= 60) continue;
				if (legal(l-1, a-1, b)) f[a][b][c] += f[a-1][b][c];
				if (legal(l-1, a, b-1)) f[a][b][c] += f[a][b-1][c];
				if (legal(l-1, a, b)) f[a][b][c] += f[a][b][c-1];
			}
		}
	}
}
